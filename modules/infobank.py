...
def create_country_league_season_round(ib_round, wf_raw_round_results, \
                                      fd_excel_positions, fd_excel_row_columns):
    """
    In - obj - country_leage object
         string - absolute path to /path/to/country/league/season/round
         string - raw round results from worldfootball
         dict - of fd_columns with their position
         list of lists - excel rows with columns
    Out - no out
    ...
    round_matches = FDH.fd_append_matches_stats(fd_excel_positions, fd_excel_row_columns, \
                                                wf_round_matches)
...
def create_country_league_season_rounds(country_league, season):
    ...
    FDH.fd_excel_download(country_league, season, fd_excel_path)
    fd_excel = FDH.fd_excel_translate_team_names(country_league, fd_excel_path)
    fd_excel_header = FDH.fd_excel_get_header(fd_excel)
    fd_excel_rows_columns = FDH.fd_excel_get_rows_columns(fd_excel)
    fd_excel_positions = FDH.fd_excel_get_positions(fd_excel_header, CFG.FD_COLUMNS)
    for play_round in play_rounds:
        ...
        if act == "create_round":
            create_country_league_season_round(ib_round, res_wf, \
                                              fd_excel_positions, fd_excel_rows_columns)
...
def create_country_league_seasons(country_league):
    ...
    for season in CFG.SEASONS:
        ...
        if create_season:
            seasons = '{0}-{1}'.format(season, season+1)
            create_item(os.path.join(CFG.IB_ROOT, country, league, seasons))
